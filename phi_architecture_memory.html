...
<head>
    ...
    <style>
        /* جميع الأنماط السابقة كما هي دون تغيير - تم حذفها من هنا للاختصار ولكنها موجودة في الكود النهائي الكامل */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Cairo', sans-serif; transition: background-color 0.3s ease, color 0.2s ease; overflow-x: hidden; scroll-behavior: smooth; }
        :root { --bg-primary: #0a0f1a; --bg-secondary: #0f1a24; --text-primary: #f0f4fa; --text-secondary: #cfdfea; --accent-gold: #e2bd6e; --accent-gold-dark: #c9a345; --card-bg: rgba(20, 30, 45, 0.75); --card-border: rgba(226, 189, 110, 0.3); --shadow-color: rgba(0,0,0,0.6); --footer-bg: #0b111c; --tribute-bg: rgba(226, 189, 110, 0.12); --input-bg: rgba(255,255,255,0.1); --message-bg: rgba(20,30,45,0.5); }
        @media (prefers-color-scheme: light) { :root { --bg-primary: #ffffff; --bg-secondary: #f8f5f0; --text-primary: #1a2c38; --text-secondary: #2c4a5e; --accent-gold: #b87c2e; --accent-gold-dark: #9b641f; --card-bg: rgba(255, 255, 250, 0.95); --card-border: rgba(184, 124, 46, 0.25); --shadow-color: rgba(0,0,0,0.08); --footer-bg: #f0ebe2; --tribute-bg: rgba(184, 124, 46, 0.08); --input-bg: rgba(0,0,0,0.04); --message-bg: #fefaf5; } body { background-color: var(--bg-primary); color: var(--text-primary); } .bg-aura { background: radial-gradient(circle at 20% 30%, rgba(220, 195, 140, 0.15), #fff6ed); } .bg-aura::after { background: repeating-linear-gradient(45deg, rgba(184,124,46,0.04) 0px, rgba(184,124,46,0.04) 1px, transparent 1px, transparent 12px); } .splash { background: #fffaf5; } .info-pills span, .faculty-gold { background: #fef7e8; color: #5a3e1f; border-color: rgba(184,124,46,0.4); } .gallery-item { background: #ffffff; backdrop-filter: none; box-shadow: 0 8px 20px rgba(0,0,0,0.05); } footer, .messages-section { background: #fefaf2; border-top-color: #e2d5c0; } .quote, .hero .tribute p { color: #2b4b60; } .lb-btn, .lb-close { background: #ffffffcc; color: #7a5a2e; backdrop-filter: blur(4px); } .hero h1 { color: #1a2c38; } .hero .sub { color: #b87c2e; } .splash-title { color: #1a2c38; } .splash-title span { color: #b87c2e; } .message-card { background: #ffffff; border: 1px solid #eee5d8; } }
        .bg-aura { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -2; transition: background 0.3s; }
        .bg-aura::after { content: ''; position: absolute; width: 150%; height: 150%; top: -25%; left: -25%; pointer-events: none; animation: slowDrift 40s linear infinite; }
        @keyframes slowDrift { 0% { transform: translate(0, 0); } 100% { transform: translate(60px, 60px); } }
        .splash { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 20000; display: flex; justify-content: center; align-items: center; flex-direction: column; transition: opacity 0.8s cubic-bezier(0.2, 0.9, 0.4, 1.1), visibility 0.8s; }
        .splash.hide { opacity: 0; visibility: hidden; }
        .splash-content { text-align: center; animation: goldGlow 1s ease; max-width: 85%; }
        @keyframes goldGlow { 0% { opacity: 0; transform: scale(0.92); filter: blur(8px); } 100% { opacity: 1; transform: scale(1); filter: blur(0); } }
        .splash-content img { max-width: 100%; max-height: 65vh; border-radius: 48px; box-shadow: 0 25px 45px var(--shadow-color); border: 1px solid var(--accent-gold); }
        .splash-title { font-size: 2rem; font-weight: 700; color: var(--text-primary); margin-top: 25px; }
        .splash-title span { display: block; font-size: 1.2rem; font-weight: normal; color: var(--accent-gold); margin-top: 8px; }
        .hero { text-align: center; padding: 100px 20px 70px; position: relative; z-index: 2; }
        .hero h1 { font-size: 2.8rem; font-weight: 700; color: var(--text-primary); letter-spacing: -0.5px; line-height: 1.3; }
        .hero .sub { font-size: 1.2rem; font-weight: 500; color: var(--accent-gold); margin-top: 15px; }
        .hero .tribute { max-width: 750px; margin: 25px auto 0; padding: 24px 28px; background: var(--tribute-bg); border-radius: 32px; backdrop-filter: blur(4px); border-right: 4px solid var(--accent-gold); text-align: right; }
        .hero .tribute p { font-size: 1rem; line-height: 1.8; color: var(--text-secondary); font-weight: 500; }
        .hero .tribute .small-line { font-size: 0.9rem; color: var(--accent-gold); margin-top: 12px; font-weight: 600; }
        .info-pills { display: flex; justify-content: center; gap: 20px; margin-top: 40px; flex-wrap: wrap; }
        .info-pills span { backdrop-filter: blur(12px); padding: 8px 28px; border-radius: 60px; font-weight: 600; font-size: 0.85rem; border: 1px solid var(--card-border); transition: 0.2s; }
        .gold-divider { width: 100px; height: 3px; background: linear-gradient(90deg, var(--accent-gold), var(--accent-gold-dark)); margin: 40px auto 0; border-radius: 3px; }
        .gallery-header { text-align: center; margin: 30px 0 50px; }
        .gallery-header h2 { font-size: 2.3rem; font-weight: 600; color: var(--text-primary); }
        .gallery-header p { color: var(--text-secondary); margin-top: 12px; font-weight: 500; }
        .gallery-header:after { content: ''; display: block; width: 70px; height: 2px; background: var(--accent-gold); margin: 18px auto 0; }
        .gallery { max-width: 1400px; margin: 0 auto; padding: 0 20px 80px; display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 28px; }
        .gallery-item { position: relative; border-radius: 32px; overflow: hidden; cursor: pointer; transition: all 0.5s cubic-bezier(0.2, 0.9, 0.4, 1.1); background: var(--card-bg); backdrop-filter: blur(4px); border: 1px solid var(--card-border); transform: translateY(0); opacity: 0; animation: cardRise 0.7s forwards; animation-delay: calc(var(--i) * 0.02s); }
        @keyframes cardRise { from { opacity: 0; transform: translateY(40px); } to { opacity: 1; transform: translateY(0); } }
        .gallery-item:hover { transform: translateY(-12px); border-color: var(--accent-gold); box-shadow: 0 30px 50px -15px var(--shadow-color); }
        .gallery-item img { width: 100%; height: 300px; object-fit: cover; display: block; transition: transform 0.7s ease, filter 0.4s; }
        .gallery-item:hover img { transform: scale(1.04); filter: brightness(1.02); }
        .glass-overlay { position: absolute; bottom: 0; left: 0; right: 0; height: 30%; background: linear-gradient(to top, rgba(0,0,0,0.5), transparent); opacity: 0; transition: 0.3s; }
        .gallery-item:hover .glass-overlay { opacity: 1; }
        .messages-section { max-width: 900px; margin: 20px auto 60px; padding: 30px 25px; background: var(--footer-bg); border-radius: 48px; box-shadow: 0 15px 35px var(--shadow-color); border: 1px solid var(--card-border); }
        .messages-title { text-align: center; margin-bottom: 30px; }
        .messages-title h3 { font-size: 1.8rem; color: var(--accent-gold); display: inline-flex; align-items: center; gap: 12px; }
        .message-input-area { display: flex; flex-direction: column; gap: 16px; margin-bottom: 40px; }
        .message-input-area input, .message-input-area textarea { background: var(--input-bg); border: 1px solid var(--card-border); border-radius: 28px; padding: 14px 22px; font-family: 'Cairo', sans-serif; font-size: 1rem; color: var(--text-primary); resize: vertical; transition: 0.2s; }
        .message-input-area input:focus, .message-input-area textarea:focus { outline: none; border-color: var(--accent-gold); box-shadow: 0 0 0 2px rgba(226,189,110,0.3); }
        .message-input-area input::placeholder, .message-input-area textarea::placeholder { color: var(--text-secondary); opacity: 0.6; }
        .send-btn { background: var(--accent-gold); border: none; border-radius: 60px; padding: 12px 24px; font-size: 1rem; font-weight: 700; color: #1a2c38; cursor: pointer; transition: 0.2s; display: inline-flex; align-items: center; justify-content: center; gap: 10px; width: 180px; margin: 0 auto; }
        .send-btn:hover { background: var(--accent-gold-dark); transform: scale(1.02); }
        .messages-list { display: flex; flex-direction: column; gap: 20px; max-height: 500px; overflow-y: auto; padding: 5px; }
        .message-card { background: var(--message-bg); border-radius: 28px; padding: 18px 22px; border-right: 4px solid var(--accent-gold); transition: 0.2s; }
        .message-header { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
        .message-header i { font-size: 1.8rem; color: var(--accent-gold); }
        .message-name { font-weight: 700; font-size: 1.1rem; color: var(--accent-gold); }
        .message-date { font-size: 0.7rem; color: var(--text-secondary); margin-right: auto; }
        .message-text { line-height: 1.6; font-size: 0.95rem; color: var(--text-primary); word-break: break-word; }
        .empty-messages { text-align: center; padding: 40px; color: var(--text-secondary); font-style: italic; }
        @media (max-width: 750px) { .hero h1 { font-size: 2rem; } .gallery { gap: 18px; grid-template-columns: repeat(auto-fill, minmax(260px,1fr)); } .gallery-item img { height: 240px; } .messages-section { margin: 20px 15px 50px; padding: 20px; } }
        .lightbox { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(5, 8, 15, 0.98); backdrop-filter: blur(25px); z-index: 30000; display: flex; justify-content: center; align-items: center; visibility: hidden; opacity: 0; transition: visibility 0.2s, opacity 0.4s ease; }
        .lightbox.active { visibility: visible; opacity: 1; }
        .lb-inner { position: relative; width: 90%; height: 90%; display: flex; justify-content: center; align-items: center; }
        .lb-img { max-width: 90vw; max-height: 85vh; object-fit: contain; border-radius: 28px; box-shadow: 0 25px 45px rgba(0,0,0,0.6); transition: opacity 0.2s, transform 0.3s; transform: scale(0.96); border: 1px solid var(--accent-gold); }
        .lightbox.active .lb-img { transform: scale(1); }
        .lb-btn { position: absolute; top: 50%; transform: translateY(-50%); backdrop-filter: blur(12px); width: 56px; height: 56px; border-radius: 60px; display: flex; align-items: center; justify-content: center; font-size: 2rem; cursor: pointer; transition: 0.2s; border: 1px solid var(--accent-gold); background: var(--card-bg); color: var(--accent-gold); }
        .lb-btn:hover { background: var(--accent-gold); color: #0a0f1a; transform: translateY(-50%) scale(1.08); }
        .lb-prev { left: 25px; } .lb-next { right: 25px; }
        .lb-close { position: absolute; top: 30px; right: 40px; backdrop-filter: blur(8px); width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 1.8rem; transition: 0.2s; border: 1px solid var(--accent-gold); background: var(--card-bg); color: var(--accent-gold); }
        .lb-close:hover { background: var(--accent-gold); color: #0a0f1a; }
        .lb-counter { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); backdrop-filter: blur(8px); padding: 8px 24px; border-radius: 60px; color: #f5e6c4; font-weight: 600; font-size: 0.9rem; }
        .admin-bar { display: flex; justify-content: flex-end; padding: 15px 25px; gap: 15px; align-items: center; position: relative; z-index: 100; }
        .admin-btn { background: var(--accent-gold); border: none; padding: 8px 20px; border-radius: 40px; font-weight: bold; cursor: pointer; color: #1a2c38; transition: 0.2s; }
        .admin-btn:hover { background: var(--accent-gold-dark); transform: scale(1.02); }
        ...
        .modal-box { background: var(--bg-secondary); border-radius: 40px; padding: 30px; width: 90%; max-width: 500px; text-align: center; border: 1px solid var(--accent-gold); color: var(--text-primary); }
        .admin-panel { max-width: 900px; width: 95%; max-height: 85vh; overflow-y: auto; }
        .image-grid-admin { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px,1fr)); gap: 12px; margin: 20px 0; }
        .img-admin-item { background: var(--card-bg); border-radius: 16px; padding: 8px; }
        ...
        .message-admin-item { background: var(--card-bg); border-radius: 24px; padding: 12px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; }
        footer { padding: 60px 20px 50px; text-align: center; border-top: 1px solid var(--card-border); background: var(--footer-bg); transition: background 0.3s; }
        .quote { max-width: 750px; margin: 0 auto; font-size: 1rem; line-height: 1.9; color: var(--text-secondary); font-weight: 500; }
        .faculty-gold { display: inline-block; padding: 10px 32px; border-radius: 60px; margin-top: 25px; font-weight: 600; border: 1px solid var(--accent-gold); background: var(--card-bg); color: var(--accent-gold); }
        small { display: block; margin-top: 35px; opacity: 0.7; font-size: 0.7rem; color: var(--text-secondary); }
    </style>
</head>
<body>
...
<script>
    ...
    const FULL_IMAGES_LIST = [
        ...
        "https://i.postimg.cc/3RMtKgNc/bbdb4381-fd4f-4f31-945c-4171bc0e1995.jpg",
        "https://i.postimg.cc/KzX0Zn4S/CAMJ0168.jpg",
        "https://i.postimg.cc/Tw8tRrh1/CUCD4601.jpg",
        "https://i.postimg.cc/6qccLgcC/IMG-0045.avif",
        "https://i.postimg.cc/8PktPkkM/IMG-0903.avif",
        "https://i.postimg.cc/SNQDNQQj/IMG-1009.avif",
        "https://i.postimg.cc/qMBjMBBx/IMG-1013.avif",
        "https://i.postimg.cc/kXjwVrDj/IMG-1034-Copy.avif",
        "https://i.postimg.cc/25WxXVG2/IMG-1074.avif",
        "https://i.postimg.cc/0NSCHbZs/IMG-1136.avif",
        "https://i.postimg.cc/DzGcY86R/IMG-2012.avif",
        "https://i.postimg.cc/V6cFymHQ/IMG-2022.avif",
        "https://i.postimg.cc/tTtNpwLr/IMG-2467.avif",
        "https://i.postimg.cc/cHMB02Vz/IMG-2702.avif",
        "https://i.postimg.cc/Ss7fk3wH/IMG-3164.avif",
        "https://i.postimg.cc/D0vdZ5hh/IMG-3185-Copy.avif",
        "https://i.postimg.cc/yxLhS96n/IMG-3188.avif",
        "https://i.postimg.cc/PfyQcGWg/IMG-3215-Copy.avif",
        "https://i.postimg.cc/HWz9KG0p/IMG-3218-Copy.avif",
        "https://i.postimg.cc/xj537rGq/IMG-3258.avif",
        "https://i.postimg.cc/pXdJM0kG/IMG-3264.avif",
        "https://i.postimg.cc/L60B9z0x/IMG-3558.avif",
        "https://i.postimg.cc/qMZXJ85T/IMG-4432.avif",
        "https://i.postimg.cc/K8k7RR6G/IMG-4433.avif",
        "https://i.postimg.cc/bv0ttbLM/IMG-4655.avif",
        "https://i.postimg.cc/Dz3G05dn/IMG-4757.avif",
        "https://i.postimg.cc/DzQXXsBV/IMG-4871.avif",
        "https://i.postimg.cc/hG8ddxMH/IMG-4920.avif",
        "https://i.postimg.cc/xCtNsQrV/IMG-4945.avif",
        "https://i.postimg.cc/L4QZLmBs/IMG-4991.avif",
        "https://i.postimg.cc/tJJZwcyJ/IMG-5793.avif",
        "https://i.postimg.cc/qRkCXR5s/IMG-6479.jpg",
        "https://i.postimg.cc/XJVycJz1/IMG-6495.jpg",
        "https://i.postimg.cc/90kRhBFt/IMG-6856.avif",
        "https://i.postimg.cc/nrrjgWZx/IMG-6993.jpg",
        "https://i.postimg.cc/2yybMtmF/IMG-7110.jpg",
        "https://i.postimg.cc/5NHQmDns/IMG-7208.avif",
        "https://i.postimg.cc/FsfkxwTm/IMG-7244-Copy.avif",
        "https://i.postimg.cc/90kRhBFk/IMG-7291.jpg",
        "https://i.postimg.cc/hPHzZMYD/IMG-7771-Copy.avif",
        "https://i.postimg.cc/2Shqshpx/IMG-7871.avif",
        "https://i.postimg.cc/8CGFNcN8/IMG-7964.avif",
        "https://i.postimg.cc/s2zBsxsC/IMG-8112.avif",
        "https://i.postimg.cc/8CGFNcD9/IMG-9001.avif",
        "https://i.postimg.cc/VNwSmvm8/IMG-9085.avif",
        "https://i.postimg.cc/431YdGxV/IMG-9092.avif",
        "https://i.postimg.cc/0NCzQvyY/IMG-9122.avif",
        "https://i.postimg.cc/ydQWmT9N/IMG-9129.avif",
        "https://i.postimg.cc/RFsq7Tf3/IMG-9210.avif",
        "https://i.postimg.cc/1XbtT5vQ/IMG-9468.avif",
        "https://i.postimg.cc/MHgT4KdC/IMG-9806.avif",
        "https://i.postimg.cc/WpyhtXTX/IMG-9807.avif",
        "https://i.postimg.cc/L8K535qL/IMG-E2467.avif",
        "https://i.postimg.cc/5tftDrSy/IMG-E3185-Copy.avif",
        "https://i.postimg.cc/1zyzbdrm/IMG-E4379.avif",
        "https://i.postimg.cc/766ZZTRp/IMG-E4433.avif",
        "https://i.postimg.cc/0QB2Mygh/IMG-E7244.avif",
        "https://i.postimg.cc/G25h8mwB/IMG-E9210.avif",
        "https://i.postimg.cc/Cdyh7rKh/IMG-E9474.avif",
        "https://i.postimg.cc/jdNPbGPN/IMG-E9514.avif",
        "https://i.postimg.cc/vBNHgZFm/JZPU2818.jpg",
        "https://i.postimg.cc/PxRrp5sP/MGNP0942.jpg",
        "https://i.postimg.cc/wvGTyj8R/MPGY7367.jpg",
        "https://i.postimg.cc/90wXGHC9/Whats-App-Image-2026-04-25-at-18-58-44.jpg",
        "https://i.postimg.cc/76WYGZ82/Whats-App-Image-2026-04-25-at-18-58-44-(1).jpg",
        "https://i.postimg.cc/vTx8Wwbg/Whats-App-Image-2026-04-25-at-18-58-44-(2).jpg",
        "https://i.postimg.cc/2yLj4Nrq/Whats-App-Image-2026-04-25-at-18-58-44-(3).jpg",
        "https://i.postimg.cc/3N0rgQYS/Whats-App-Image-2026-04-25-at-18-58-45.jpg",
        "https://i.postimg.cc/L5g4zF2L/Whats-App-Image-2026-04-25-at-18-58-45-(1).jpg",
        "https://i.postimg.cc/NF2G1vB8/Whats-App-Image-2026-04-25-at-18-58-45-(2).jpg",
        "https://i.postimg.cc/NF2G1vB4/Whats-App-Image-2026-04-25-at-18-58-45-(3).jpg",
        "https://i.postimg.cc/Lhd9DQ8v/Whats-App-Image-2026-04-25-at-18-58-50.jpg",
        "https://i.postimg.cc/yx36FCsG/Whats-App-Image-2026-04-25-at-18-58-50-(1).jpg",
        "https://i.postimg.cc/BtWSp7vW/Whats-App-Image-2026-04-25-at-18-59-37.jpg",
        "https://i.postimg.cc/qgfJwDvD/Whats-App-Image-2026-04-25-at-18-59-37-(1).jpg",
        "https://i.postimg.cc/3WM81fxV/Whats-App-Image-2026-04-25-at-18-59-37-(2).jpg",
        "https://i.postimg.cc/hhFDsZGt/Whats-App-Image-2026-04-25-at-18-59-38.jpg",
        "https://i.postimg.cc/23pkG95f/Whats-App-Image-2026-04-25-at-18-59-38-(1).jpg",
        "https://i.postimg.cc/tRsFr1gV/Whats-App-Image-2026-04-25-at-19-01-02.jpg",
        "https://i.postimg.cc/7P5SBCLG/Whats-App-Image-2026-04-25-at-19-01-02-(1).jpg",
        "https://i.postimg.cc/XNtdtVGb/Whats-App-Image-2026-04-25-at-19-01-03.jpg",
        "https://i.postimg.cc/sfM5nB2p/Whats-App-Image-2026-04-25-at-19-01-03-(1).jpg",
        "https://i.postimg.cc/ZYW6f9qH/Whats-App-Image-2026-04-25-at-19-01-03-(2).jpg",
        "https://i.postimg.cc/dQhGW70W/Whats-App-Image-2026-04-25-at-19-01-03-(3).jpg",
        "https://i.postimg.cc/fWVYH3bq/Whats-App-Image-2026-04-25-at-19-01-03-(4).jpg",
        "https://i.postimg.cc/xjXLskdh/Whats-App-Image-2026-04-25-at-19-01-03-(5).jpg",
        "https://i.postimg.cc/59DLDxHs/Whats-App-Image-2026-04-25-at-19-01-03-(6).jpg",
        "https://i.postimg.cc/bY4k4zGB/Whats-App-Image-2026-04-25-at-19-01-03-(7).jpg",
        "https://i.postimg.cc/kMLKLnVL/Whats-App-Image-2026-04-25-at-19-01-03-(8).jpg",
        "https://i.postimg.cc/Pf0Y0tps/Whats-App-Image-2026-04-25-at-19-01-03-(9).jpg",
        "https://i.postimg.cc/7Y70kp06/Whats-App-Image-2026-04-25-at-19-01-04.jpg",
        "https://i.postimg.cc/bY4k4zDp/Whats-App-Image-2026-04-25-at-19-01-04-(1).jpg",
        "https://i.postimg.cc/dQgrgqkQ/Whats-App-Image-2026-04-25-at-19-01-04-(2).jpg",
        "https://i.postimg.cc/fTXmN6mW/Whats-App-Image-2026-04-25-at-19-01-04-(3).jpg",
        "https://i.postimg.cc/c4YwSVws/Whats-App-Image-2026-04-25-at-19-01-04-(4).jpg",
        "https://i.postimg.cc/WbrgvQg8/Whats-App-Image-2026-04-25-at-19-06-43.jpg",
        "https://i.postimg.cc/qMnyT9yC/Whats-App-Image-2026-04-25-at-19-06-43-(1).jpg",
        "https://i.postimg.cc/9FTZhvZ7/Whats-App-Image-2026-04-25-at-19-06-43-(2).jpg",
        "https://i.postimg.cc/sDS7rt7J/Whats-App-Image-2026-04-25-at-19-06-43-(3).jpg",
        "https://i.postimg.cc/MpSRKdmm/Whats-App-Image-2026-04-25-at-19-06-44.jpg",
        "https://i.postimg.cc/qvTnMjxK/Whats-App-Image-2026-04-25-at-19-06-44-(1).jpg",
        "https://i.postimg.cc/kgm8XfvL/Whats-App-Image-2026-04-26-at-00-09-09.jpg",
        "https://i.postimg.cc/7Zk7Ymnm/Whats-App-Image-2026-04-26-at-00-09-09-(1).jpg",
        "https://i.postimg.cc/wjQNtLwr/Whats-App-Image-2026-04-26-at-00-09-09-(2).jpg",
        "https://i.postimg.cc/P5QDLY6H/Whats-App-Image-2026-04-26-at-00-09-10.jpg",
        "https://i.postimg.cc/bw9nGkmB/Whats-App-Image-2026-04-26-at-00-09-10-(1).jpg",
        "https://i.postimg.cc/W1wZDg5K/Whats-App-Image-2026-04-26-at-00-09-10-(2).jpg",
        "https://i.postimg.cc/MpmBcVsM/Whats-App-Image-2026-04-26-at-00-09-11.jpg",
